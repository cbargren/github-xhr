...
var { XHR_TIMEOUT_DEFAULT } = require("./constants");
...
var Client = module.exports = function(config) {
  ...
  config.timeout = config.timeout || XHR_TIMEOUT_DEFAULT;
  this.config = config;
...
(function() {
  ...
  this.httpSend = function(msg, block, callback) {
    ...
    var query = obj.query;
    var path = this.config.url ? this.config.url + obj.url : obj.url;
    ...
    function httpSendRequest() {
      ...
      xhr.onreadystatechange = function onreadystatechange() {
        ...
        if (xhr.status >= 300 && xhr.status <= 399 && 'location' in responseHeaders) {
          ...
          if (self.debug) {
            console.log(`redirecting to ${redirectUrl}`); // eslint-disable-line no-console
          }
          url = redirectUrl;
        }
        ...
        if (xhr.status >= 400 && xhr.status < 600 || xhr.status < 10) {
          callCallback(new error.HttpError(responseBody, xhr.status, responseHeaders));
        } else {
      ...
      xhr.onerror = function onerror(err) {
        callCallback(err, xhr);
      };

      xhr.ontimeout = function ontimeout() {
        if (self.debug) {
          console.log('problem with request: timed out'); // eslint-disable-line no-console
        }
